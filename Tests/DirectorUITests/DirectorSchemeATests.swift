...
final class DirectorSchemeATests: XCTestCase {
    ...
    func testHomeAndSettingsUseSharedSchemeATonesAndPrimaryAction() throws {
        ...
        XCTAssertTrue(settings.contains("return version ?? \"1.1.1\""))
        XCTAssertFalse(settings.contains("1.1.1 (23)"))
    ...
    func testApplicationVersionContractAdvancesWithoutMovingHarnessVersion() throws {
        ...
        XCTAssertEqual(pbxproj.components(separatedBy: "MARKETING_VERSION = 1.1.1").count - 1, 2)
        XCTAssertEqual(pbxproj.components(separatedBy: "CURRENT_PROJECT_VERSION = 23").count - 1, 2)
    ...
    func testVersionAndExportCopyAreSynchronizedAcrossPublicSources() throws {
        ...
        XCTAssertEqual(pbxproj.components(separatedBy: "MARKETING_VERSION = 1.1.1").count - 1, 2)
        XCTAssertEqual(pbxproj.components(separatedBy: "CURRENT_PROJECT_VERSION = 23").count - 1, 2)
        XCTAssertTrue(readClient.contains("requestPayload(version: \"1.1.1\")"))
        XCTAssertTrue(validation.contains("CapabilityPackageProducer(version: \"1.1.1\", build: \"23\")"))
        XCTAssertTrue(changelog.contains("## 1.1.1"))
        XCTAssertTrue(changelog.contains("## 1.1.0"))
        XCTAssertTrue(changelog.contains("## 1.0.0"))
        XCTAssertTrue(readme.contains("1.1.1"))
        XCTAssertTrue(readmeChinese.contains("1.1.1"))
        XCTAssertFalse(readme.contains("1.1.1 (23)"))
        XCTAssertFalse(readmeChinese.contains("1.1.1 (23)"))
