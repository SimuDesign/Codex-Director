...
final class DirectorSchemeATests: XCTestCase {
    ...
    func testHomeAndSettingsUseSharedSchemeATonesAndPrimaryAction() throws {
        ...
        XCTAssertTrue(settings.contains("return version ?? \"1.1.0\""))
        XCTAssertFalse(settings.contains("1.1.0 (22)"))
    ...
    func testApplicationVersionContractAdvancesWithoutMovingHarnessVersion() throws {
        ...
        XCTAssertEqual(pbxproj.components(separatedBy: "MARKETING_VERSION = 1.1.0").count - 1, 2)
        XCTAssertEqual(pbxproj.components(separatedBy: "CURRENT_PROJECT_VERSION = 22").count - 1, 2)
    ...
    func testVersionAndExportCopyAreSynchronizedAcrossPublicSources() throws {
        ...
        XCTAssertEqual(pbxproj.components(separatedBy: "MARKETING_VERSION = 1.1.0").count - 1, 2)
        XCTAssertEqual(pbxproj.components(separatedBy: "CURRENT_PROJECT_VERSION = 22").count - 1, 2)
        XCTAssertTrue(readClient.contains("requestPayload(version: \"1.1.0\")"))
        XCTAssertTrue(validation.contains("CapabilityPackageProducer(version: \"1.1.0\", build: \"22\")"))
        XCTAssertTrue(changelog.contains("## 1.1.0"))
        XCTAssertTrue(changelog.contains("## 1.0.0"))
        XCTAssertTrue(readme.contains("1.1.0"))
        XCTAssertTrue(readmeChinese.contains("1.1.0"))
        XCTAssertFalse(readme.contains("1.1.0 (22)"))
        XCTAssertFalse(readmeChinese.contains("1.1.0 (22)"))
