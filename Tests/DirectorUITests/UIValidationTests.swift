...
final class UIValidationTests: XCTestCase {
    ...
    func testCaptureModeHidesControlsAndUsesEscapeToRestoreThem() throws {
        ...
        XCTAssertTrue(host.contains(".modifier(UIValidationCaptureExitCommand(isCaptureMode: isCaptureMode, exitCaptureMode: exitCaptureMode))"))
        let captureModifierStart = try XCTUnwrap(host.range(of: "private struct UIValidationCaptureExitCommand: ViewModifier"))
        let captureModifierEnd = try XCTUnwrap(host[captureModifierStart.upperBound...].range(of: "/// Debug-only native host")?.lowerBound)
        let captureModifier = String(host[captureModifierStart.lowerBound..<captureModifierEnd])
        XCTAssertTrue(captureModifier.contains("if isCaptureMode {\n            content.onExitCommand(perform: exitCaptureMode)\n        } else {\n            content\n        }"), "normal validation must return the unmodified content, not register a nil Escape handler")
        XCTAssertEqual(host.components(separatedBy: "onExitCommand").count - 1, 1)
        XCTAssertFalse(host.contains("captureExitCommand: (() -> Void)?"))
