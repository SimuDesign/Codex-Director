...
> 当前开发版本：`1.3.0`。需要 macOS 26 或更高版本。
...
“能力分类”页面会把当前索引中的 Agent 和 Skill 按用途整理到固定的本地分类中，包括视频制作、UI 设计、软件开发、内容创作、研究与数据、自动化与效率、通用工具和未分类。自动分类只使用名称与声明用途的确定性规则，不调用 AI 或网络；你可以新增、重命名和删除自建分类，也可以手动调整单项能力并随时恢复自动分类。

分类设置只保存自建分类名称、稳定能力 ID 和手动覆盖关系，不保存路径、能力正文或会话数据。系统能力、插件本体、MCP、工具、指令和旧插件缓存不会进入分类页面。
