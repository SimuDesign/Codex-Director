...
type/sort filters, keyboard-accessible move actions, drag-and-drop ordering for
custom folders, and manual local folder operations. Verify new Self Training is
empty, while existing memberships are preserved and never implicitly re-added.
Verify exact declarations only, separate near-seven-day co-observation evidence,
preview-only outside-folder relations, and reverse Agent links from Skill
details. Verify each custom folder's Add existing capabilities sheet stages a
...
- Cover cache present/fresh, cache expired, cache absent with an existing index, and an empty installation. The native window, all seven destinations and Settings remain usable; only unknown values show a pending state.
...
Expected: seven local dates; same-cycle increases become daily percentage-point use; a reset day retains observed use before and after the boundary; missing, cross-gap and ambiguous transitions remain unavailable; confirmed flat observations may show 0%. A source with only a five-hour window shows “weekly quota history unavailable” and never creates weekly bars from short-window data. The named segmented source control switches ring and bars together. There is no Token conversion, recorded-time/evidence clutter, cross-source mixing or ambiguous reset label, and values remain readable without color/hover.
...
Test all seven pages and representative details in zh/en, Light/Dark, minimum720×480/default1280×800/wide. Verify actual-content breakpoints760/1000; compact numbers never split, selected controls keep readable values, chart text and native roles survive AX inspection. On Capability Folders, exercise the global search result list, open and return from Global/Project/custom folder cards, switch all folder scopes through the fixed Agent & Companion Skills / Agent / Skill tabs, combine folder search and sort controls, import several existing capabilities into a custom folder with one confirmation, toggle multi-membership from rows and details, verify custom-folder drag and keyboard move actions, and confirm separate empty-folder/no-match/import-failure states. Verify the toolbar refresh button in idle, source/projection loading and failed-recovery states; its text never disappears, loading uses native `ProgressView`, and Settings uses the same state. Complete hover, keyboard focus/activation, back/Escape, evaluation and settings flows under standard and Increase Contrast/Reduce Motion conditions. Confirm Settings action geometry is 176pt content width by 48pt outer height and that no localized label is truncated. Record VoiceOver reading separately from AX-tree inspection; untested cases must be listed, not inferred from unit tests.
...
5. Repeat with no index: navigate all seven destinations and manually start indexing; no synthetic inventory or fabricated zeros appear.
...
- Shared refresh button visible-label, 14pt native-progress/symbol slot, stable-width and standard/toolbar-size contracts. Resolve the actual dynamic colors, sample the complete normal/hover/pressed/disabled action rails, and require at least `4.5:1`: white content in Light and black content in Dark. Disabled styling must not depend on layered opacity compositing.
...
all capability pages and Settings. Check the central Light/Dark accent
...
exposes the seven approved destinations for this matrix.

Focused source/tests cover shared component construction, breakpoints, Home
shared-component consumption, all capability-library tones and filters,
...
stable control width, theme-specific action content, hover, focus, disabled state,
...
verify the template gauge and exact persistent formats: `5h 82% w 57%` for two
valid windows, a bare `82%` or `57%` for one, and `—` for none. Inspect the
native window in the fixed order: five-hour remaining/reset when available,
weekly remaining/reset when available, reset-card count, Refresh data, Open
main window. Expire either window independently and verify that only that
window's ring, label and popover rows disappear. Verify the same matrix in
zh/en × Light/Dark, with the main window visible and closed, missing/stale/
expired data, account-read loading, failure recovery and explicit zero reset
cards. VoiceOver and the AX tree must
...
## 14. Light UI optimization matrix — 2026-09-12
...
- Compare evidence show/hide and refresh idle/loading/disabled outer geometry, label baselines and native progress proportions. Exercise pointer activation, Escape and arrow selection; record AX inspection and actual VoiceOver speech as separate evidence.

Increase Contrast, Reduce Transparency, Reduce Motion and VoiceOver remain release gates unless each was actually exercised. Do not infer them from standard-setting AX output or unit tests. P3 compact-density item 09 is outside this matrix.
